...
olm.install: ## Install OLM to your cluster
	...
	@docker run -v ${KUBECONFIG}:/root/.kube/config -v ${PWD}/community-operators:/community-operators -v ${PWD}/upstream-community-operators:/upstream-community-operators -it quay.io/operator-framework/operator-testing olm.install --no-print-directory VERBOSE=${VERBOSE}

operator.install:
	@scripts/ci/run-script "docker pull quay.io/operator-framework/operator-testing" "Pulling docker image"
	@python3 scripts/utils/check-kube-config.py
	@docker run -v ${KUBECONFIG}:/root/.kube/config -v ${PWD}/community-operators:/community-operators -v ${PWD}/upstream-community-operators:/upstream-community-operators -ti quay.io/operator-framework/operator-testing operator.install --no-print-directory OP_PATH=${OP_PATH} VERBOSE=${VERBOSE} OP_VER=${OP_VER} OP_CHANNEL=${OP_CHANNEL} INSTALL_MODE=${INSTALL_MODE} CLEAN_MODE=${CLEAN_MODE}
...
operator.test: check_path check_kind ## Operator test which run courier and scorecard
	...
	@docker run --network=host -v ${KUBECONFIG}:/root/.kube/config -v ${PWD}/community-operators:/community-operators -v ${PWD}/upstream-community-operators:/upstream-community-operators -ti quay.io/dmesser/operator-testing operator.test --no-print-directory OP_PATH=${OP_PATH} VERBOSE=${VERBOSE} OP_VER=${OP_VER} OP_CHANNEL=${OP_CHANNEL} INSTALL_MODE=${INSTALL_MODE} CLEAN_MODE=${CLEAN_MODE} OLM_VER=${OLM_VER} KUBE_VER=${KUBE_VER} NO_KIND=${NO_KIND} CATALOG_IMAGE=${CATALOG_IMAGE}
