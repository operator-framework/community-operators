...
operator.verify: check_path ## Run only courier
	@docker run -v ./community-operators:/community-operators -v ./upstream-community-operators:/upstream-community-operators -ti sebastiansimko/operator-command operator.verify --no-print-directory OP_PATH=${OP_PATH} VERBOSE=${VERBOSE}
